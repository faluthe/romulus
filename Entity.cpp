...
WeaponEntity* PlayerEntity::activeWeapon()
{
	auto activeWeapon{ *reinterpret_cast<uintptr_t*>(this + netvars::activeWeapon) & 0xFFF };
	return interfaces::entityList->GetClientEntity<WeaponEntity>(activeWeapon);
}

void LocalplayerEntity::aimAt(Vector pos, CUserCmd* cmd, bool silent)
{
	Vector angle;
